...
import com.banquet.acs_banquet1.service.ProductsService;
...
public class ProductsController {
    ...
    public Products createProducts(@RequestPart("wm_data_json") Products products, @RequestPart(value = "ppicture", required = false) MultipartFile _ppicture) {
		LOGGER.debug("Create Products with information: {}" , products);

    products.setPpicture(WMMultipartUtils.toByteArray(_ppicture));
    ...
    @ApiOperation(value = "Retrieves content for the given BLOB field in Products instance" )
    @RequestMapping(value = "/{id}/content/{fieldName}", method = RequestMethod.GET, produces="application/octet-stream")
    @WMAccessVisibility(value = AccessSpecifier.APP_ONLY)
    public DownloadResponse getProductsBLOBContent(@PathVariable("id") Integer id, @PathVariable("fieldName") String fieldName, HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, @RequestParam(value="download", defaultValue = "false") boolean download) {

        LOGGER.debug("Retrieves content for the given BLOB field {} in Products instance" , fieldName);

        if(!WMRuntimeUtils.isLob(Products.class, fieldName)) {
            throw new TypeMismatchException("Given field " + fieldName + " is not a valid BLOB type");
        }
        Products products = productsService.getById(id);

        return WMMultipartUtils.buildDownloadResponseForBlob(products, fieldName, httpServletRequest, download);
    }
    ...
    @ApiOperation(value = "Updates the Products instance associated with the given id.This API should be used when Products instance fields that require multipart data.") 
    @RequestMapping(value = "/{id:.+}", method = RequestMethod.POST, consumes = {"multipart/form-data"})
    @WMAccessVisibility(value = AccessSpecifier.APP_ONLY)
    public Products editProducts(@PathVariable("id") Integer id, MultipartHttpServletRequest multipartHttpServletRequest) throws EntityNotFoundException {
        Products newProducts = WMMultipartUtils.toObject(multipartHttpServletRequest, Products.class, "acs_banquet1");
        newProducts.setId(id);

        Products oldProducts = productsService.getById(id);
        WMMultipartUtils.updateLobsContent(oldProducts, newProducts);
        LOGGER.debug("Updating Products with information: {}" , newProducts);

        return productsService.update(newProducts);
    }
