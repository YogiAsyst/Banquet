...
import com.banquet.acs_banquet1.User;
 ...
 * @see User
 */
@Service("acs_banquet1.UserService")
@Validated
public class UserServiceImpl implements UserService {
    ...
    @Autowired
    @Qualifier("acs_banquet1.UserDao")
	...
	public User update(User user) throws EntityNotFoundException {
        ...
        if(packageEntities != null && Hibernate.isInitialized(packageEntities) && !packageEntities.isEmpty()) {
            List<PackageEntity> _remainingChildren = wmGenericDao.execute(
                session -> DaoUtils.findAllRemainingChildren(session, PackageEntity.class,
                        new DaoUtils.ChildrenFilter("user", user, packageEntities)));
            LOGGER.debug("Found {} detached children, deleting", _remainingChildren.size());
            for(PackageEntity _packageEntity : _remainingChildren) {
                packageEntityService.delete(_packageEntity);
            }
            user.setPackageEntities(packageEntities);
        }

        // Deleting children which are not present in the list.
        if(_orders != null && Hibernate.isInitialized(_orders) && !_orders.isEmpty()) {
            List<Order> _remainingChildren = wmGenericDao.execute(
                session -> DaoUtils.findAllRemainingChildren(session, Order.class,
                        new DaoUtils.ChildrenFilter("user", user, _orders)));
            LOGGER.debug("Found {} detached children, deleting", _remainingChildren.size());
            for(Order _order : _remainingChildren) {
                orderService.delete(_order);
            }
            user.set_orders(_orders);
        }
    ...
    public Downloadable export(ExportType exportType, String query, Pageable pageable) {
        LOGGER.debug("exporting data in the service acs_banquet1 for table User to {} format", exportType);
